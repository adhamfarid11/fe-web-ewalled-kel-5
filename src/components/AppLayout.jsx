...
const AppLayout = ({ children, isAuthenticated, handleLogout, mobileMenuOpen, setMobileMenuOpen }) => {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50'} flex flex-col transition-colors duration-200`}>
      {showNavbar && (
        <header className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-b w-full transition-colors duration-200`}>
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
            <div className="flex items-center space-x-2">
              <div className={`flex items-center ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>
                ...
                <span className="ml-2 font-semibold">Walled</span>
              </div>
            </div>
            ...
            <nav className="hidden md:flex items-center space-x-8">
              ...
              <a href="/transfer" onClick={(e) => { e.preventDefault(); handleNavigation('/transfer'); }}
                 className={`${location.pathname === '/transfer'
                   ? (darkMode ? 'text-blue-400 font-medium' : 'text-blue-600 font-medium')
                   : (darkMode ? 'text-gray-300' : 'text-gray-600')}`}>
                Transfer
              </a>
              <a href="/topup" onClick={(e) => { e.preventDefault(); handleNavigation('/topup'); }}
                 className={`${location.pathname === '/topup'
                   ? (darkMode ? 'text-blue-400 font-medium' : 'text-blue-600 font-medium')
                   : (darkMode ? 'text-gray-300' : 'text-gray-600')}`}>
                Topup
              </a>
              <button onClick={handleLogout} className={darkMode ? 'text-gray-300' : 'text-gray-600'}>Sign Out</button>
          ...
          {mobileMenuOpen && (
            <div className={`md:hidden border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} w-full`}>
              <div className="container mx-auto px-4 pt-2 pb-3 space-y-1">
                <a href="/" onClick={(e) => { e.preventDefault(); handleNavigation('/'); }}
                   className={`block px-3 py-2 rounded-md text-base font-medium ${
                     location.pathname === '/'
                     ? (darkMode ? 'text-blue-400 bg-gray-800' : 'text-blue-600 bg-gray-50')
                ...
                <div className={`my-2 h-px w-full ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
                ...
                <div className="flex items-center px-3 py-2">
                  ...
                  <button
                    onClick={toggleDarkMode}
                    className={`ml-auto p-2 rounded-full ${darkMode ? 'bg-gray-700 text-yellow-300' : 'bg-gray-100 text-gray-700'}`}
                    aria-label={darkMode ? 'Switch to light mode' : 'Switch to dark mode'}
                  >
                    {darkMode ? <Sun size={20} /> : <Moon size={20} />}
                  </button>
      ...
      <main className={`flex-grow w-full ${!showNavbar ? 'p-0' : 'container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8'}`}>
        {children}
      </main>
    </div>
  );
